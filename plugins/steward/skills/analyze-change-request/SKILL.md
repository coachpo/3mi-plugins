...
tools. Give workers the question, relevant target and version, authorized read
scope, and analysis-only boundary. The main agent reconciles the results and
checks decisive evidence directly. If delegation is unavailable, research
directly. Tool failures or inaccessible sources warrant a fallback or an
explicit evidence gap, not invented support.

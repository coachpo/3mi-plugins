...
from model import AdapterError, CampaignError
...
class AdapterV2Tests(unittest.TestCase):
    ...
    def test_unsupported_shapes_are_rejected(self) -> None:
        cases = [
            ...
            (
                lambda value: value["source"].update({"excludes": []}),
                "must contain .steward",
            ),
            (
                lambda value: value["cases"][0].update({"required": False}),
                "required-case coverage",
            ),
            (
                lambda value: value["cases"][0].update({"coversCriteria": ["C2"]}),
                "unknown GOAL criterion",
            ),
            (
                lambda value: value["cases"][0].update(
                    {"coversCriteria": ["C1", "C1"]}
                ),
                "duplicate values",
            ),
    ...
    def test_symlink_source_is_rejected(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(CampaignError, "symlink"):
                observe_source(validate_adapter(path))

    def test_source_inventory_requires_present_non_control_project_content(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            path = make_project(root)
            (root / "app.txt").unlink()
            with self.assertRaisesRegex(CampaignError, "non-control project source"):
                observe_source(validate_adapter(path))
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            with self.assertRaisesRegex(CampaignError, "must declare project source"):
                observe_source(validate_adapter(path))

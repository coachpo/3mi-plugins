...
def make_project(
    ...
    *,
    ...
    adapter = {
        ...
        "projectId": "sample",
        "projectRoot": "..",
        "campaignRoot": ".steward/verification/campaign",
        "source": {
            "provider": "files",
            "files": ["app.txt"],
            "excludes": [".steward"],
...
def run_cli(adapter: Path, command: str, *arguments: str) -> subprocess.CompletedProcess[str]:
    environment = os.environ.copy()
    environment["PYTHONDONTWRITEBYTECODE"] = "1"
    return subprocess.run(
        [sys.executable, "-B", str(CAMPAIGN), command, "--adapter", str(adapter), *arguments],
        check=False,
        capture_output=True,
        text=True,
        env=environment,

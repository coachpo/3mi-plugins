...
Treat a `write-project-docs` Current Iteration Strategy as source-current only after its read-only validator confirms the block content digest and all four bound source digests for `STATUS.md`, the selected product document, architecture document, and development rules. Then use its positive requirements and non-negotiable boundaries together with the Goal and reachable risks to decide which actual purposes need required cases. Do not add a case only because a hypothetical future state could make it useful, and do not turn deferred work into exclusion IDs, digests, or audit proof. In full mode all five tier categories still apply to the current purpose, but the tier labels do not by themselves require load, capacity, high-availability, or production-scale testing. Narrow mode may omit a tier only as explicitly reported out of scope. A `full` campaign covers the whole current catalog, not every imaginable future scenario; user requirements, hard invariants, and supported reachable review findings still override a deferred description.

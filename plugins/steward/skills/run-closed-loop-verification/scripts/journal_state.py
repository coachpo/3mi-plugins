"""Append-only schema-5 campaign journal and deterministic projections."""
...
EVENT_FIELDS = {
    "schemaVersion", "kernelVersion", "sequence", "timestamp", "type", "payload",
    "previousHash", "eventHash",
}
EVENT_PAYLOAD_FIELDS = {
    "initialized": {
        "adapterFingerprint", "projectId", "goalSnapshot", "sourceSnapshot", "cases",
        "repairPolicy", "runtimePlatform", "worktreeBinding",
    },
    "attempt_started": {"attemptId", "mode", "sourceFingerprint", "caseIds"},
    "case_started": {
        "attemptId", "runId", "caseId", "ordinal", "artifactDir", "sourceFingerprint",
    },
    "case_finished": {
        "attemptId", "runId", "caseId", "ordinal", "artifactDir", "status", "reason",
        "exitCode", "timedOut", "evidence", "stdoutSha256", "stderrSha256",
        "sourceFingerprint", "sourceAfterFingerprint", "artifactManifest",
    },
...
OWNED_FILES = {"events.jsonl", "state.json", "summary.json", "campaign.lock"}
...
def _validate_event(event: Any, expected_sequence: int, previous_hash: str | None) -> None:
    if not isinstance(event, dict) or set(event) != EVENT_FIELDS:
        raise CampaignError("journal event has invalid fields")
    if type(event["schemaVersion"]) is not int or event["schemaVersion"] != JOURNAL_SCHEMA_VERSION:
        raise CampaignError("journal schemaVersion must be 5")
    if event["kernelVersion"] != SCRIPT_VERSION:
        raise CampaignError("journal kernelVersion must be 0.5.0")
    ...
    if not isinstance(payload, dict) or set(payload) != EVENT_PAYLOAD_FIELDS[event_type]:
...
def read_events(path: Path) -> list[dict[str, Any]]:
    content = read_regular_bytes(path, label="campaign journal", max_bytes=MAX_JOURNAL_BYTES)
...
def _new_event(
    ...
    if event_type not in EVENT_PAYLOAD_FIELDS or set(payload) != EVENT_PAYLOAD_FIELDS[event_type]:
...
def _case_state_map(cases: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    return {
        case["id"]: {
            "status": "PENDING", "lastRunId": None, "lastAttemptId": None,
            "lastRound": None, "reason": None,
        }
        for case in cases
    }
...
def _initial_state(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(payload["runtimePlatform"], str) or not payload["runtimePlatform"]:
    ...
    return {
        ...
        "repairPolicy": payload["repairPolicy"],
        "repairCount": 0,
        "status": "PENDING",
        "resumeMode": "initial",
...
def apply_event(state: dict[str, Any] | None, event: dict[str, Any]) -> dict[str, Any]:
    ...
    if event_type == "initialized":
        ...
        state["attempts"].append({
            "id": payload["attemptId"], "mode": payload["mode"],
            "sourceFingerprint": payload["sourceFingerprint"],
            "caseIds": list(payload["caseIds"]), "runs": [], "status": "RUNNING",
        })
        state["status"] = "RUNNING"
        state["resumeMode"] = payload["mode"]
    elif event_type == "case_started":
        attempt = _find_attempt(state, payload["attemptId"])
        if attempt["status"] != "RUNNING" or payload["caseId"] not in attempt["caseIds"]:
        ...
        attempt["runs"].append({
            "runId": payload["runId"], "caseId": payload["caseId"],
            "ordinal": payload["ordinal"], "artifactDir": payload["artifactDir"],
            "sourceFingerprint": payload["sourceFingerprint"], "status": "RUNNING",
        })
        case_state = state["caseStates"][payload["caseId"]]
        case_state.update({
            "status": "RUNNING", "lastRunId": payload["runId"],
            "lastAttemptId": payload["attemptId"], "lastRound": attempt["mode"],
            "reason": None,
        })
        ...
        case_state = state["caseStates"][payload["caseId"]]
        case_state.update({
            "status": payload["status"], "lastRunId": payload["runId"],
            "lastAttemptId": payload["attemptId"], "lastRound": attempt["mode"],
            "reason": payload["reason"],
        })
    elif event_type == "attempt_finished":
        attempt = _find_attempt(state, payload["attemptId"])
        if attempt["status"] != "RUNNING" or any(run["status"] == "RUNNING" for run in attempt["runs"]):
        ...
        if attempt["mode"] == "regression" and payload["status"] == "AUDIT_REQUIRED":
        ...
        for run in attempt["runs"]:
            if run["status"] == "RUNNING":
                ...
                state["caseStates"][run["caseId"]].update({
                    "status": "INTERRUPTED", "reason": payload["reason"],
                })
        ...
        if state["repairPolicy"] != "within-goal":
            raise CampaignError("repairPolicy does not permit source repair")
        ...
        if attempt["mode"] != "regression" or attempt["status"] != "AUDIT_REQUIRED":
            raise CampaignError("audit success is not bound to a successful regression")
        if payload["currentSourceFingerprint"] != state["sourceBaseline"]["fingerprint"]:
...
def make_summary(state: dict[str, Any]) -> dict[str, Any]:
    return {
        ...
        "caseStatuses": {
            case_id: value["status"] for case_id, value in sorted(state["caseStates"].items())
        },
...
class CampaignLock:
        ...
        for attempt in range(2):
            try:
                ...
                payload = canonical_bytes(
                    {"kernelVersion": SCRIPT_VERSION, "pid": os.getpid()}
                ) + b"\n"
                ...
                if attempt or not self._remove_stale_lock():
                    raise CampaignError("campaign is locked by another operation") from exc
            except OSError as exc:
    ...
    def _remove_stale_lock(self) -> bool:
        try:
            ...
            if (
                not isinstance(value, dict)
                or set(value) != {"kernelVersion", "pid"}
                or value["kernelVersion"] != SCRIPT_VERSION
    ...
    def __exit__(
        ...
        try:
            metadata = self.path.lstat()
            if self.identity == (metadata.st_dev, metadata.st_ino) and stat.S_ISREG(metadata.st_mode):
                self.path.unlink()
        except OSError:
            pass
...
class Campaign:
    ...
    def state_path(self) -> Path:
        return self.adapter.campaign_root / "state.json"

    @property
    def summary_path(self) -> Path:
        return self.adapter.campaign_root / "summary.json"

    @classmethod
    def initialize(cls, adapter: Adapter, repair_policy: str = "within-goal") -> Campaign:
        if repair_policy not in REPAIR_POLICIES:
            raise CampaignError("repairPolicy must be within-goal or verify-only")
        ...
        try:
            ...
            state = replay_projection([event])
            atomic_write_json(temp / "state.json", state)
            atomic_write_json(temp / "summary.json", make_summary(state))
    ...
    def load(cls, adapter: Adapter) -> Campaign:
        adapter = validate_adapter(adapter.path, observe_goal_drift=True)
        root = adapter.campaign_root
        require_safe_campaign_root(root)
        required = {"events.jsonl", "state.json", "summary.json"}
        try:
        ...
        if missing:
            raise CampaignError("campaignRoot is incomplete: missing " + ", ".join(missing))
        ...
        stored_state = read_json(root / "state.json")
        stored_summary = read_json(root / "summary.json")
        return cls(
            adapter, events, state,
            snapshot_consistent=stored_state == state,
            summary_consistent=stored_summary == make_summary(state),
    ...
    def catalog_drift_reason(self) -> str | None:
        ...
        if not self.runtime_platform_consistent:
            return "RUNTIME_PLATFORM_DRIFT: runtime platform differs from initialization"
        return None

    def ensure_mutable(self) -> None:
        if self.catalog_drift_reason():
            raise CampaignError(self.catalog_drift_reason() or "campaign drift")
        if self.state["status"] == "BLOCKED" and self.state.get("invalidation"):
            raise CampaignError("campaign is permanently blocked by source invalidation")

    def rebuild_projections(self) -> None:
        atomic_write_json(self.state_path, self.state)
        atomic_write_json(self.summary_path, make_summary(self.state))
    ...
    def commit(self, event_type: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if len(canonical_bytes(prospective)) > MAX_PROJECTION_BYTES:
            raise CampaignError("campaign projection exceeds the safe size limit")
    ...
    def allocate_case_artifact(self, attempt_id: str, case_id: str, ordinal: int) -> tuple[str, Path]:
...
__all__ = [
    "Campaign", "CampaignLock", "apply_event", "make_summary", "read_events",
    "replay_projection", "require_safe_campaign_root",
]

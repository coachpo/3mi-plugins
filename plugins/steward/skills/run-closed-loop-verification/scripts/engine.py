...
def dependency_reason(campaign: Campaign, case: dict[str, Any]) -> str | None:
    for dependency in case["dependsOn"]:
        status = campaign.state["caseStates"][dependency]["status"]
        if status not in PASS_STATUSES:
            return "dependency " + dependency + " has status " + status
    return None
...
def _initial_satisfied(campaign: Campaign) -> bool:
    for case in campaign.adapter.cases:
        ...
        if (
            not case["required"]
            and platform_supported_on(case["platform"], campaign.state["runtimePlatform"])
...
def _failed_finish(campaign: Campaign, attempt_id: str, status: str) -> dict[str, Any]:
    resume = "record-fix" if status == "FAILED" and campaign.state["repairPolicy"] == "within-goal" else None
    campaign.commit(
        "attempt_finished",
...
def _run_attempt(
    campaign: Campaign, mode: str, case_ids: list[str]
) -> dict[str, Any]:
    ...
    if current != expected_source:
        if mode == "regression":
            synthetic_id = _attempt_id(campaign, mode, expected_source)
            campaign.commit(
                "attempt_started",
                {
            ...
            return _invalidate(
                campaign, synthetic_id, "source changed before regression", current
            )
    ...
    for ordinal, case_id in enumerate(case_ids, start=1):
        ...
        if campaign.current_source() != expected_source:
            return _invalidate(
                campaign, attempt_id, "source changed during " + mode, campaign.current_source()
            )
        if not platform_supported_on(case["platform"], campaign.state["runtimePlatform"]):
            ...
            reason = dependency_reason(campaign, case)
            if reason:
                result = record_blocked_case(
                    campaign,
                    attempt_id,
...
def run_initial_locked(campaign: Campaign) -> dict[str, Any]:
    ...
    case_ids = [
        case["id"]
        for case in campaign.adapter.cases
        if campaign.state["caseStates"][case["id"]]["status"] not in PASS_STATUSES | {"NOT_RUN"}
    ]
    if not case_ids:
...
def _validate_root_cause_source(
    ...
    required = {"path", "lineStart", "lineEnd", "failedSha256"}
    if (
    ...
    if failed_metadata is None or failed_metadata.get("status") != "present":
        raise CampaignError("rootCauseSource.path must exist in the failed source snapshot")
    failed_sha256 = value["failedSha256"]
    if (
        not isinstance(failed_sha256, str)
        or failed_sha256 != failed_metadata.get("sha256")
    ):
        raise CampaignError("rootCauseSource.failedSha256 must match the failed snapshot")
    ...
    if type(line_count) is not int:
        raise CampaignError("rootCauseSource requires UTF-8 text in the failed snapshot")
    if end > line_count:
...
def record_fix_locked(campaign: Campaign, value: dict[str, Any]) -> dict[str, Any]:
    if campaign.state["repairPolicy"] != "within-goal":
        raise CampaignError("repairPolicy verify-only does not authorize source repair")
    ...
    expected_fields = {
        "schemaVersion", "failedCaseId", "failedRound", "failedAttemptId", "failedRunId",
        "failedSourceFingerprint", "fixedSourceFingerprint", "rootCause",
        "rootCauseSource", "affectedCriteria", "sourceDelta", "fixSummary", "newEvidence",
    }
    if not isinstance(value, dict) or set(value) != expected_fields:
        raise CampaignError("fix audit has invalid fields")
    if type(value["schemaVersion"]) is not int or value["schemaVersion"] != FIX_AUDIT_SCHEMA_VERSION:
        raise CampaignError("fix audit schemaVersion must be 1")
    ...
    for key, expected in bindings.items():
        if value[key] != expected:
            raise CampaignError("fix audit " + key + " does not match the latest failure")
    ...
    if value["fixedSourceFingerprint"] != after["fingerprint"]:
        raise CampaignError("fixedSourceFingerprint does not match current source")
    expected_delta = source_snapshot_changed_entries(before, after)
    delta = value["sourceDelta"]
    if not isinstance(delta, list) or not delta:
        raise CampaignError("sourceDelta must contain the complete project-source repair")
    normalized_delta: list[dict[str, str]] = []
    for item in delta:
        if not isinstance(item, dict) or set(item) != {"path", "change"}:
            raise CampaignError("sourceDelta entry has invalid fields")
        path = normalize_relative(item["path"], "sourceDelta.path")
        if item["change"] not in {"added", "modified", "deleted", "mode-only"}:
            raise CampaignError("sourceDelta.change is invalid")
        normalized_delta.append({"path": path, "change": item["change"]})
    if len({item["path"] for item in normalized_delta}) != len(normalized_delta):
        raise CampaignError("sourceDelta contains duplicate paths")
    if sorted(normalized_delta, key=lambda item: item["path"]) != expected_delta:
        raise CampaignError("sourceDelta is not the exact observed project-source delta")
    affected = value["affectedCriteria"]
    if not isinstance(affected, list) or not affected or len(affected) != len(set(affected)):
        raise CampaignError("affectedCriteria must be a non-empty unique array")
    if any(item not in campaign.adapter.goal_criteria_ids for item in affected):
        raise CampaignError("affectedCriteria contains an unknown GOAL criterion")
    ...
    if not isinstance(evidence, list) or not evidence or len(evidence) != len(set(evidence)):
        raise CampaignError("newEvidence must be a non-empty unique array")
    evidence = [_nonempty_string(item, "newEvidence item") for item in evidence]
...
__all__ = [
    "dependency_reason", "latest_failed_run", "load_fix", "record_fix_locked",
    "resume_locked", "retest_locked", "run_initial_locked", "run_regression_locked",
]

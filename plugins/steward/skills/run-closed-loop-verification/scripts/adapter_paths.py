...
def _load_worktree_binding_module():
    ...
    try:
        ...
        raise AdapterError(
            "cannot load the Steward worktree binding validator"
        ) from exc
    return module
...
def validate_evidence_contract(case_id: str, value: Any) -> dict[str, list[str]]:
    ...
    required = _unique_strings(
        value["requiredFiles"], "case " + case_id + " evidence.requiredFiles"
    )
    non_empty = _unique_strings(
        value["nonEmptyFiles"], "case " + case_id + " evidence.nonEmptyFiles"
    )
    required_normalized = [
        normalize_relative(item, "case " + case_id + " evidence file")
        for item in required
    ]
    non_empty_normalized = [
        normalize_relative(item, "case " + case_id + " non-empty evidence file")
    ...
    def criteria_configuration(self) -> list[dict[str, Any]]:
        return [
            ...
            for criterion in sorted(
                self.goal_criteria_ids, key=lambda item: int(item[1:])
            )
        ]


def _goal_snapshot(
    project_root: Path, contract: dict[str, Any], observe_drift: bool
    ...
    if (
        type(contract.get("contractVersion")) is not int
        or contract["contractVersion"] != 1
    ):
        raise AdapterError("goalContract.contractVersion must be 1")
    if (
        not isinstance(contract.get("sha256"), str)
        or HASH_PATTERN.fullmatch(contract["sha256"]) is None
    ):
        raise AdapterError("goalContract.sha256 must be a lowercase sha256 digest")
    goal_path = resolve_project_path(
        project_root, contract["path"], "goalContract.path"
    )
    ...
    try:
        ...
        if not observe_drift:
            raise AdapterError(
                "GOAL_CONTRACT_DRIFT: cannot validate .steward/goal.txt"
            ) from exc
        return {
            "path": contract["path"],
            "contractVersion": 1,
    ...
    if errors and not observe_drift:
        raise AdapterError(
            errors[0] + ": goalContract digest does not match .steward/goal.txt"
        )
    return snapshot, errors


def validate_adapter(
    adapter_path: Path, *, observe_goal_drift: bool = False, **_ignored: Any
) -> Adapter:
    ...
    _reject_nul(data)
    expected_top = {"schemaVersion", "source", "goalContract", "cases"}
    ...
    try:
        ...
        raise AdapterError(
            "cannot bind projectRoot as the exact target worktree"
        ) from exc
    ...
    campaign_root = resolve_project_path(
        project_root, ".steward/verification/campaign", "campaignRoot"
    )
    ...
    try:
        ...
        if (
            ...
            or workspace_contract.get("criteriaIds") != goal_snapshot["criteriaIds"]
        ):
    ...
    for index, case in enumerate(cases):
        ...
        if not goal_errors and not set(case["coversCriteria"]).issubset(criteria_ids):
            raise AdapterError(
                "case " + case_id + " references an unknown GOAL criterion"
            )
        argv = case["argv"]
        if (
        ...
        if any(
            secret_option.match(argv[position]) for position in range(len(argv) - 1)
        ):
            raise AdapterError(
        ...
        cwd = resolve_project_path(
            project_root, case["cwd"], "case " + case_id + " cwd"
        )
        if not cwd.is_dir():
    ...
    if not goal_errors:
        uncovered = [
            ...
            if not any(
                case["required"] and criterion in case["coversCriteria"]
                for case in cases
            )
        ]
        if uncovered:
            raise AdapterError(
                "GOAL criteria require required-case coverage: "
    ...
    adapter = Adapter(
        ...
        campaign_root,
        excludes,
        sha256_bytes(canonical_bytes(data)),
        goal_snapshot,
        goal_errors,
        worktree_binding,
        goal_workspace_errors,
    )
    return adapter
...
def _manifest_source_paths(adapter: Adapter) -> list[str]:
    manifest_relative = normalize_relative(
        adapter.data["source"]["manifest"], "source.manifest"
    )
...
def _source_entry(adapter: Adapter, relative: str) -> dict[str, Any]:
    ...
    content = read_regular_bytes(
        unresolved, label="source file", max_bytes=MAX_SOURCE_FILE_BYTES
    )
    try:
        after = unresolved.lstat()
    except OSError as exc:
        raise CampaignError(
            "source path changed while it was observed: " + relative
        ) from exc
    if (
...
def observe_source(adapter: Adapter) -> dict[str, Any]:
    entries = [
        _source_entry(adapter, relative) for relative in _declared_source_paths(adapter)
    ]
...
def source_snapshot_changed_entries(
    ...
    for path in sorted(set(old) | set(new)):
        ...
        if left.get("status") == "missing" and right.get("status") == "present":
            ...
            kind = "deleted"
        elif left.get("sha256") == right.get("sha256") and left.get(
            "executable"
        ) != right.get("executable"):
            kind = "mode-only"
        else:
            kind = "modified"

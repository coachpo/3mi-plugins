...
def failure_fix_context(
    ...
    if len(requested) > 64:
        raise CampaignError("status accepts at most 64 --source-path values")
    normalized = [normalize_relative(item, "status source path") for item in requested]
    if len(normalized) != len(set(normalized)):
        raise CampaignError("status --source-path values must be unique")
    if campaign.state["status"] != "FAILED":
        if normalized:
            raise CampaignError("status --source-path requires a FAILED campaign")
        return None, None
    for attempt in reversed(campaign.state["attempts"]):
        for run in reversed(attempt["runs"]):
            ...
            by_path = {
                item["path"]: item
                for item in campaign.state["sourceBaseline"]["files"]
            }
            unknown = [path for path in normalized if path not in by_path]
            if unknown:
                raise CampaignError(
                    "status --source-path is not in the failed source baseline: "
                    + ", ".join(unknown)
                )
            files = []
            for path in normalized:
                ...
                if "sha256" in item:
                    projected["failedSha256"] = item["sha256"]
            ...
            return (
                ...
                },
                {
                    ...
                    "failedSourceFileCount": len(by_path),
...
def status_report(
    campaign: Campaign, source_paths: list[str] | None = None
) -> dict[str, Any]:
    ...
    latest_failure, fix_context = failure_fix_context(campaign, source_paths)
    return {
        ...
        "repairPolicy": campaign.state["repairPolicy"],
        ...
        "projectionConsistent": campaign.snapshot_consistent
        and campaign.summary_consistent,
...
def _audit_binding_errors(campaign: Campaign) -> list[str]:
    ...
    if campaign.state["status"] == "AUDIT_REQUIRED":
        return [] if binding is None else ["audit binding exists before durable completion"]
    if campaign.state["status"] != "COMPLETE":
        return [] if binding is None else ["audit binding exists in a non-complete state"]
...
def _artifact_errors(campaign: Campaign, run: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(binding, dict) or set(binding) != {"relativePath", "size", "sha256"}:
        return ["run has an invalid artifact-manifest binding"]
    relative = binding["relativePath"]
    if not isinstance(relative, str) or not relative.endswith("/artifact-manifest.json"):
    ...
    for path, metadata in entries:
        ...
        if stat.S_ISDIR(metadata.st_mode):
            continue
        if stat.S_ISLNK(metadata.st_mode) or artifact_metadata_is_reparse(metadata) or not stat.S_ISREG(metadata.st_mode):
    ...
    for item in manifest["files"]:
        if not isinstance(item, dict) or set(item) != {"relativePath", "size", "sha256"}:
...
def _final_case_errors(campaign: Campaign, final: dict[str, Any]) -> tuple[list[str], list[str]]:
    errors: list[str] = []
    codes: list[str] = []
    expected_ids = [case["id"] for case in campaign.adapter.cases]
    if final.get("caseIds") != expected_ids or [run["caseId"] for run in final.get("runs", [])] != expected_ids:
        errors.append("final regression does not contain the complete ordered case catalog")
    ...
    for case, run in zip(campaign.adapter.cases, final["runs"]):
        runnable = platform_supported_on(case["platform"], campaign.state["runtimePlatform"])
        expected = "PASS" if case["required"] or runnable else "NOT_RUN"
        if run["status"] != expected:
            errors.append("final regression case " + case["id"] + " has status " + run["status"])
            codes.append("REQUIRED_CASES_INCOMPLETE")
        if run["sourceFingerprint"] != baseline or run["sourceAfterFingerprint"] != baseline:
            errors.append("final regression case " + case["id"] + " is not bound to the final source")
...
def audit_report(campaign: Campaign) -> dict[str, Any]:
    codes: list[str] = []
    errors: list[str] = []
    if campaign.adapter.goal_errors or campaign.adapter.goal_snapshot != campaign.state["goalSnapshot"]:
    ...
    if not campaign.worktree_binding_consistent:
        ...
        errors.append("the current target worktree identity differs from initialization")
    ...
    if not campaign.snapshot_consistent or not campaign.summary_consistent:
        codes.append("JOURNAL_PROJECTION_MISMATCH")
        errors.append("state.json or summary.json differs from authoritative journal replay")
    ...
    final = _final_attempt(campaign)
    if final is None or final.get("mode") != "regression" or final.get("status") != "AUDIT_REQUIRED":
    ...
    for criterion in criterion_coverage(campaign):
        if not criterion["finalPassingCaseIds"]:
            ...
            errors.append("GOAL criterion lacks a required final-PASS case: " + criterion["id"])
...
__all__ = [
    "audit_report", "campaign_coverage", "completion_status", "criterion_coverage",
    "failure_fix_context", "status_report",
]

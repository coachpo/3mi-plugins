...
def _read_stdin() -> bytes:
    if sys.stdin.buffer.isatty():
        raise VerificationError("INPUT_TRANSPORT", "structured input must arrive through a finite non-TTY pipe")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Verify one alias-scoped Steward GOAL in the current worktree.")
    sub = parser.add_subparsers(dest="command", required=True)
    for name in ("init", "status", "advance", "record-repair"):
        command = sub.add_parser(name)
        command.add_argument("--goal", required=True, help="Safe alias below .steward/goals/")
        if name == "init":
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print("ERROR VERIFICATION: interrupted; run advance to resume the phase", file=sys.stderr)
        return 130

...
def _redact_bytes(content: bytes) -> Tuple[bytes, bool]:
    ...
    if decoded_found and not found:
        # A future text-only pattern that cannot be mapped safely to original
        # bytes must never cause the unredacted artifact to be persisted.
        return b"<REDACTED_UNMAPPABLE_CONTENT>\n", True
...
def execute_case(
    ...
    if execution_safety_error:
        ...
        status = "PASS"
        reason = None
    if status == "PASS" and round_mode == "retest":

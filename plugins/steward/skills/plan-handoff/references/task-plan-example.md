...
the acceptance record stays empty until `INT-01` is reviewed. The executor
protocol follows the task cards unchanged.
...
- **Source and dependencies:** the accepted request above; no predecessor
  output.
- **Executor tier:** `strong`; the skill's wording needs judgment within the
  fixed checks.
- **Entry check:** from the repository root,
  `git diff --quiet 478e595 -- plugins/steward/.claude-plugin plugins/steward/.codex-plugin .claude-plugin/marketplace.json`
  exits 0 and `plugins/steward/skills/review-steward-metadata` does not exist.
...
- **Attempt budget:** two fix-and-validate cycles for validator failures, then
...
- **Result:** pending. Assign a result ID/version when work is recorded; bind
  ...
  the commit and diff summary, each command with its exit code and output,
  findings, status, and the next action after interruption.
...
- **Executor tier:** `basic`; placement and wording are the only judgment, and
  every check is mechanical.
- **Entry check:** `META-01` is `done`, and
  `git diff --quiet 478e595 -- README.md plugins/steward/README.md` exits 0.
...
- **Attempt budget:** two fix-and-validate cycles, then rerun as `strong`.
...
- **Executor tier:** `basic`; every check is a command or a comparison.
- **Entry check:** both tasks are `done`, and `git status --porcelain` lists
  only files named in their results.
...
- **Attempt budget:** two fix-and-validate cycles inside the accepted cards,
  then rerun as `strong`.
